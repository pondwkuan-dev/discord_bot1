...
async def setup_hook():
    ...
    # ทำให้ปุ่มปิด Ticket ยังใช้งานได้หลังบอทรีสตาร์ต
...
class TicketView(discord.ui.View):
    ...
    async def close_ticket(
        ...
        # อนุญาตให้เจ้าของ Ticket หรือผู้ที่มี Manage Channels ปิดได้
        is_manager = interaction.user.guild_permissions.manage_channels
        is_ticket_owner = (
            interaction.user in interaction.channel.overwrites
            and interaction.channel.permissions_for(interaction.user).view_channel
        )

        if not (is_manager or is_ticket_owner):
            await interaction.response.send_message(
                "❌ คุณไม่มีสิทธิ์ปิด Ticket นี้",
                ephemeral=True
            )
            return
        ...
        try:
            await interaction.channel.delete(
                reason=f"ปิด Ticket โดย {interaction.user}"
            )
        except discord.Forbidden:
            # ต้องให้ JUZEN BOT มีสิทธิ์ Manage Channels
            pass
        except Exception:
            pass
